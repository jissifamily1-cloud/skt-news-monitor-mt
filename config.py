...
KEYWORDS = [
    ...
    "배경훈",        # 과기정통부 장관
    "AI수석",        # 대통령실 AI미래기획수석
    "하정우",        # AI미래기획수석 (배우 동명이인은 EXCLUDED_WORDS로 차단)
    "한성숙",        # 중소벤처기업부 장관
    "국가AI전략위",  # 국가AI전략위원회(부분일치)
]
...
EXCLUDED_WORDS = [
    ...
    "고동빈 감독",
    "감독 고동빈",
    # 배우 하정우 — AI수석(인물 하정우)와 동명이인 오탐 차단
    "배우 하정우",
    "하정우 배우",
    "하정우 감독",
    "하정우 주연",
...
# 발송 허용 언론사 화이트리스트 — 기사 도메인이 여기에 없으면 발송하지 않음.
# host == domain 또는 서브도메인(.domain)까지 매칭.
# chosun.com은 조선비즈(biz.)·IT조선(it.) 서브도메인까지 함께 포함.
ALLOW_DOMAINS = [
    "chosun.com",         # 조선일보 (+조선비즈 biz., IT조선 it.)
    "joongang.co.kr",     # 중앙일보
    "donga.com",          # 동아일보
    "hani.co.kr",         # 한겨레
    "khan.co.kr",         # 경향신문
...
# 발송 허용 시간대 (KST, 24h). 이 시간대 밖에서는 수집·발송하지 않음.
# 6, 22 → 06:00~21:59 발송, 22:00~05:59 발송 안 함(야간 제외).
# 아침 첫 실행도 당일 06:00 이후 발행분만 보냄(밤사이 기사 몰아보기 안 함).
ACTIVE_START_HOUR = 6
ACTIVE_END_HOUR = 22
